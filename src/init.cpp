...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: lcv.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: lcvd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizelcvamount=<n> " + _("Keep N LocavoreCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. LocavoreCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. LocavoreCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("LocavoreCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "LocavoreCoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of LocavoreCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart LocavoreCoin to complete") << "\n";
    ...
    nAnonymizeLocavoreCoinAmount = GetArg("-anonymizelcvamount", 0);
    if(nAnonymizeLocavoreCoinAmount > 999999) nAnonymizeLocavoreCoinAmount = 999999;
    if(nAnonymizeLocavoreCoinAmount < 2) nAnonymizeLocavoreCoinAmount = 2;
    ...
    LogPrintf("Anonymize LocavoreCoin Amount %d\n", nAnonymizeLocavoreCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1LCV+1000 == (.1LCV+100)*10
       10LCV+10000 == (1LCV+1000)*10
    */
