...
//
// Bootup the masternode, look for a 2500 LCV input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in lcv.conf */
...
{
    CLocavoreCoincoinAddress address(collateralAddress);
