...
public:
                      ...
                      const QModelIndex &index ) const
    {
        ...
        //if(variant.canConvert(QMetaType::QColor))
        //{
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeLocavoreCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeLocavoreCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeLocavoreCoinAmount = strAnonymizeLocavoreCoinAmount.remove(strAnonymizeLocavoreCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLocavoreCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeLocavoreCoinAmount*COIN) nMaxToAnonymize = nAnonymizeLocavoreCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeLocavoreCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeLocavoreCoinAmount));
        strAnonymizeLocavoreCoinAmount = strAnonymizeLocavoreCoinAmount.remove(strAnonymizeLocavoreCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLocavoreCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeLocavoreCoinAmount == 0){
