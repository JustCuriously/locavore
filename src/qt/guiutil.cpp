...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("lcv://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "lcv:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "LocavoreCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for LocavoreCoin.lnk
...
{
    return GetAutostartDir() / "lcv.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a lcv.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=LocavoreCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the lcv app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add lcv app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("LocavoreCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  lcv-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("LocavoreCoin-Qt"));
