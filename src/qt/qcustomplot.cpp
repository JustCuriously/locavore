...
/* end documentation of inline functions */
/* start documentation of lcv virtual functions */
...
/* end documentation of lcv virtual functions */
...
/*! \class QCPAbstractPlottable
  ...
  To create an own plottable, you implement a subclass of QCPAbstractPlottable. These are the lcv
...
/* start of documentation of lcv virtual functions */
...
/* end of documentation of lcv virtual functions */
/* start of documentation of signals */
...
/*! \class QCPAbstractItem
  ...
  To create an own item, you implement a subclass of QCPAbstractItem. These are the lcv
  virtual functions, you must implement:
  \li \ref selectTest
  \li \ref draw
...
/*! \class QCPAbstractLegendItem
  ...
  You must implement the following lcv virtual functions:
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const
{
  ...
  // pointers to be able to swap them, depending which data range needs lcvping:
  QVector<QPointF> *staticData = &thisData;
  QVector<QPointF> *lcvpedData = &otherData;
  
  // lcv both vectors to ranges in which the keys overlap (which coord is key, depends on axisType):
  if (keyAxis->orientation() == Qt::Horizontal)
  {
    // x is key
    ...
    if (lcvpedData->first().x() > lcvpedData->last().x())
    {
      int size = lcvpedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*lcvpedData)[i], (*lcvpedData)[size-1-i]);
    }
    // lcv lower bound:
    if (staticData->first().x() < lcvpedData->first().x()) // other one must be lcvped
      qSwap(staticData, lcvpedData);
    int lowBound = findIndexBelowX(lcvpedData, staticData->first().x());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    lcvpedData->remove(0, lowBound);
    // set lowest point of lcvped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (lcvpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (lcvpedData->at(1).x()-lcvpedData->at(0).x() != 0)
      slope = (lcvpedData->at(1).y()-lcvpedData->at(0).y())/(lcvpedData->at(1).x()-lcvpedData->at(0).x());
    else
      slope = 0;
    (*lcvpedData)[0].setY(lcvpedData->at(0).y()+slope*(staticData->first().x()-lcvpedData->at(0).x()));
    (*lcvpedData)[0].setX(staticData->first().x());
    
    // lcv upper bound:
    if (staticData->last().x() > lcvpedData->last().x()) // other one must be lcvped
      qSwap(staticData, lcvpedData);
    int highBound = findIndexAboveX(lcvpedData, staticData->last().x());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    lcvpedData->remove(highBound+1, lcvpedData->size()-(highBound+1));
    // set highest point of lcvped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (lcvpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = lcvpedData->size()-1; // last index
    if (lcvpedData->at(li).x()-lcvpedData->at(li-1).x() != 0)
      slope = (lcvpedData->at(li).y()-lcvpedData->at(li-1).y())/(lcvpedData->at(li).x()-lcvpedData->at(li-1).x());
    else
      slope = 0;
    (*lcvpedData)[li].setY(lcvpedData->at(li-1).y()+slope*(staticData->last().x()-lcvpedData->at(li-1).x()));
    (*lcvpedData)[li].setX(staticData->last().x());
  } else // mKeyAxis->orientation() == Qt::Vertical
  {
    ...
    if (lcvpedData->first().y() < lcvpedData->last().y())
    {
      int size = lcvpedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*lcvpedData)[i], (*lcvpedData)[size-1-i]);
    ...
    // lcv lower bound:
    if (staticData->first().y() > lcvpedData->first().y()) // other one must be lcvped
      qSwap(staticData, lcvpedData);
    int lowBound = findIndexAboveY(lcvpedData, staticData->first().y());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    lcvpedData->remove(0, lowBound);
    // set lowest point of lcvped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (lcvpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (lcvpedData->at(1).y()-lcvpedData->at(0).y() != 0) // avoid division by zero in step plots
      slope = (lcvpedData->at(1).x()-lcvpedData->at(0).x())/(lcvpedData->at(1).y()-lcvpedData->at(0).y());
    else
      slope = 0;
    (*lcvpedData)[0].setX(lcvpedData->at(0).x()+slope*(staticData->first().y()-lcvpedData->at(0).y()));
    (*lcvpedData)[0].setY(staticData->first().y());
    
    // lcv upper bound:
    if (staticData->last().y() < lcvpedData->last().y()) // other one must be lcvped
      qSwap(staticData, lcvpedData);
    int highBound = findIndexBelowY(lcvpedData, staticData->last().y());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    lcvpedData->remove(highBound+1, lcvpedData->size()-(highBound+1));
    // set highest point of lcvped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (lcvpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = lcvpedData->size()-1; // last index
    if (lcvpedData->at(li).y()-lcvpedData->at(li-1).y() != 0) // avoid division by zero in step plots
      slope = (lcvpedData->at(li).x()-lcvpedData->at(li-1).x())/(lcvpedData->at(li).y()-lcvpedData->at(li-1).y());
    else
      slope = 0;
    (*lcvpedData)[li].setX(lcvpedData->at(li-1).x()+slope*(staticData->last().y()-lcvpedData->at(li-1).y()));
    (*lcvpedData)[li].setY(staticData->last().y());
  }
