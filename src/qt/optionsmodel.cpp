...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeLocavoreCoinAmount"))
        settings.setValue("nAnonymizeLocavoreCoinAmount", 1000);
    nAnonymizeLocavoreCoinAmount = settings.value("nAnonymizeLocavoreCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeLocavoreCoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeLocavoreCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeLocavoreCoinAmount:
            return QVariant(nAnonymizeLocavoreCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeLocavoreCoinAmount:
            nAnonymizeLocavoreCoinAmount = value.toInt();
            settings.setValue("nAnonymizeLocavoreCoinAmount", nAnonymizeLocavoreCoinAmount);
            emit AnonymizeLocavoreCoinAmountChanged(nAnonymizeLocavoreCoinAmount);
            break;
        default:
            break;
        }
